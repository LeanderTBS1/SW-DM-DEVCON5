...
class SQLiteHandler:
    ...
    def create_tables(self):
        ...
        CREATE TABLE IF NOT EXISTS sensor (
            sensor_id INTEGER PRIMARY KEY,  -- sensor_id is now the primary key
    ...
    def insert_sensor_type(self, sensor_id, sensor_type_name):
        self.cursor.execute("SELECT COUNT(*) FROM sensor WHERE sensor_id = ?", (sensor_id,))
        count = self.cursor.fetchone()[0]
        
        self.cursor.execute("""
            INSERT OR IGNORE INTO sensor (sensor_id, type_name)
            VALUES (?, ?)
    ...
    def insert_sds_data(self, sds_data):
        ...
        INSERT INTO sds_data (sensor_id, location_id, timestamp, p1, dur_p1, ratio_p1, p2, dur_p2, ratio_p2) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);
        """, sds_data)
        self.conn.commit()
    ...
    def insert_dht_data(self, dht_data):
        self.cursor.executemany("""
        INSERT INTO dht_data (sensor_id, location_id, timestamp, temperature, humidity) 
        VALUES (?, ?, ?, ?, ?);
        """, dht_data)
        self.conn.commit()
    ...
    def insert_data_from_csv(self):
      ...
      for row in sds_reader:
          ...
          if sensor_id is not None and sensor_type_name:
              self.db_handler.insert_sensor_type(sensor_id, sensor_type_name)
